...
- [Multimodal LLM Interpretability](#multimodal-llm-interpretability)
  ...
  - [Использование](#использование)
    ...
    - [5. Возможные ошибки](#5-возможные-ошибки)
  - [Контакты](#контакты)
  - [Ссылки](#ссылки)
...
В своей работе я провожу исследования с моделью `llava-1.5-7b`. Следующий код запускает модель и создает интерактивный HTML файл для иллюстрации работы logit lens.

* `CUDA_VISIBLE_DEVICES=0,1 python3 scripts/create_logit_lens.py --image_folder ./images --save_folder ./save_folder --device auto` если на одной GPU меньше 16Gb видеопамяти. Флагом `CUDA_VISIBLE_DEVICES=0,1` указываем те карты, которые будут использоваться.
...
### 5. Возможные ошибки
- Я получил HTML-файл, но не вижу результата работы программы.
  *Ответ:* убедитесь, что вы нажали "Trust HTML", если открываете через Jupyter Notebook. 
